...
- Invokes `/doc-agent`
- Mentions terms like: *test plan, test case, test checklist, test documentation, traceability matrix, bug report template, confluence page, generate docs, export to docx, write to confluence*
...
2. If pandoc is available, write the document to a temporary `.md` file in `d:\Work\CLAUDE\ProjectPay`, then convert:
   ...
   ```
   Skip `--reference-doc` if no reference template exists.
...
6. File naming convention: `[DocType]_[FeatureName]_[YYYY-MM-DD].docx`
   Example: `TestPlan_Payments_2026-05-26.docx`
